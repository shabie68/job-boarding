    ...
    const getUnreadNotifications = () => {
        apiClient.get('http://127.0.0.1:8000/api/get-unread-notifications').then(response => {
            if(response?.data?.total_notifications) {
                setNotificationsCount(response?.data?.total_notifications)
        ...
        <BoardJobContext.Provider value={boardJob} >
            ...
            <BrowserRouter>
                <div className="gorgeous-position d-flex bj-gradient-footer w-100 top-0 bj-px-16 mb-3 text-light menubar-links bj-space-between bj-align-items-center">
                    ...
                    >
                        <span className="gorgeous-notifications position-relative">
                            <span className="gorgeous-notifications-count">
                                {notificationsCount} 
                            </span>
                        ...
                        <div className="text-decoration-none text-light">Notifications</div>
                    </div>
                ...
                <div className="container">
                            ...
                            <ul style={{listStyle: 'none', padding: 0}}>
                                {
                                    notifications?.map((notification, idx) => (
                                        ...
                                        <li key={'notification-' + notification.id} style={{padding: '8px', borderBottom: idx+1 != notifications.length ? "1px solid lightgray" : "none"}}>
                                            <div className="d-flex">
                                                ...
                                                <span style={{marginLeft:"auto", textAlign: "end", width: '15%', color: "#6c757d"}}>{timeAgo(notification.created_at)}</span>
                                            </div>
                    ...
                    <Routes>
                        <Route path="/home" element={<ShowJob setNotifications={setNotifications} setNotificationsCount={setNotificationsCount} notificationsCount={notificationsCount} user={user} updateMessageContext={updateMessageContext} updateJobContext={updateJobContext} closeMenu={closeMenu}/>} />
